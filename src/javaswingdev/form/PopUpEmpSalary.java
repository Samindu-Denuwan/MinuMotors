...
public class PopUpEmpSalary extends javax.swing.JFrame {

    private Logger log;
    private Paysheet paysheet;

    public PopUpEmpSalary() {
        initComponents();
        setLocationRelativeTo(null);

        initLogger();
        log = Logger.getLogger("bikeLogger");
        log.info("Salary Emp Popup Page1 Loaded");
        table.fixTable(jScrollPane1);
        loadTable();

    }

    PopUpEmpSalary(Paysheet paysheet) {
        this();
    ...
    private void initComponents() {
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Employee ID", "Name", "Basic Salary", "Job Role", "Last Paid"
            }
        ) {
    ...
    private void tableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableMouseClicked
        try {
            ...
            String emp_name = dtm.getValueAt(selectRow, 1).toString();
            String role = dtm.getValueAt(selectRow, 3).toString();
            String salary = dtm.getValueAt(selectRow, 2).toString();

            paysheet.txt_emp_id.setText(emp_id);
            paysheet.txt_emp_name.setText(emp_name);
            paysheet.txt_job_role.setText(role);
            paysheet.txt_basic.setText(salary);
            dispose();

        } catch (Exception e) {
            log.error("Load emp Details Failed", e);
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PopUpEmpSalary.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new PopUpEmpSalary().setVisible(true);
            }
        });
    }
    ...
    private void loadTable() {
        try {
            ...
            DefaultTableModel dtm = (DefaultTableModel) table.getModel();
            dtm.setRowCount(0);
            ResultSet rs = DB.search("select s.emp_id, s.basic_salary, "
                    + "s.stat, e.fname, e.lname, e.role_id,"
                    + " r.role from basic_salary as s, employee as e"
                    + " , role as r where s.emp_id = e.emp_id &&"
                    + " e.role_id = r.role_id && s.status = 1");
            while (rs.next()) {
                Vector v = new Vector();
