package javaswingdev.form;

import com.bike.common.SystemConfigData;
import com.bike.db.DB;
import java.sql.ResultSet;
import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.Vector;
import javax.swing.JOptionPane;
import javax.swing.RowFilter;
import javax.swing.table.DefaultTableModel;
import javax.swing.table.TableRowSorter;
import org.apache.log4j.Logger;

public class ServiceInfo extends javax.swing.JPanel {

    private Logger log;

    public ServiceInfo() {
        initComponents();
        log = Logger.getLogger("bikeLogger");
        log.info("Manage Service Page Loaded");
        JT_service.fixTable(jScrollPane1);
        generateServiceID();
        loadServTable();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        roundPanel1 = new javaswingdev.swing.RoundPanel();
        jLabel6 = new javax.swing.JLabel();
        delete = new javax.swing.JButton();
        jLabel2 = new javax.swing.JLabel();
        txt_service_id = new javax.swing.JTextField();
        txt_service_name = new javax.swing.JTextField();
        jLabel3 = new javax.swing.JLabel();
        save = new javax.swing.JButton();
        update = new javax.swing.JButton();
        jLabel7 = new javax.swing.JLabel();
        jLabel9 = new javax.swing.JLabel();
        jLabel14 = new javax.swing.JLabel();
        jLabel15 = new javax.swing.JLabel();
        jLabel16 = new javax.swing.JLabel();
        jLabel1 = new javax.swing.JLabel();
        jLabel5 = new javax.swing.JLabel();
        jLabel11 = new javax.swing.JLabel();
        txt_amount = new javax.swing.JTextField();
        jLabel4 = new javax.swing.JLabel();
        clearField = new javax.swing.JLabel();
        switchButton = new javaswingdev.swing.SwitchButton();
        roundPanel2 = new javaswingdev.swing.RoundPanel();
        card1 = new javaswingdev.card.Card();
        jLabel13 = new javax.swing.JLabel();
        textFieldAnimation1 = new swing.TextFieldAnimation();
        jScrollPane1 = new javax.swing.JScrollPane();
        JT_service = new javaswingdev.swing.table.Table();

        setBackground(new java.awt.Color(228, 232, 229));
        setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        roundPanel1.setBackground(new java.awt.Color(255, 255, 255));
        roundPanel1.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        jLabel6.setFont(new java.awt.Font("Dialog", 1, 24)); // NOI18N
        jLabel6.setText("Service Info");
        roundPanel1.add(jLabel6, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 20, -1, -1));

        delete.setBackground(new java.awt.Color(153, 204, 255));
        delete.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        delete.setForeground(new java.awt.Color(0, 102, 204));
        delete.setText("DELETE");
        delete.setBorder(null);
        delete.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                deleteActionPerformed(evt);
            }
        });
        roundPanel1.add(delete, new org.netbeans.lib.awtextra.AbsoluteConstraints(200, 550, 160, 35));

        jLabel2.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        jLabel2.setForeground(new java.awt.Color(102, 102, 102));
        jLabel2.setText("Service  ID");
        roundPanel1.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 100, -1, -1));

        txt_service_id.setEditable(false);
        txt_service_id.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        txt_service_id.setForeground(new java.awt.Color(102, 102, 102));
        txt_service_id.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txt_service_id.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 153, 204)));
        roundPanel1.add(txt_service_id, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 130, 340, 35));

        txt_service_name.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        txt_service_name.setForeground(new java.awt.Color(102, 102, 102));
        txt_service_name.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txt_service_name.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 153, 204)));
        roundPanel1.add(txt_service_name, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 220, 340, 35));

        jLabel3.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        jLabel3.setForeground(new java.awt.Color(102, 102, 102));
        jLabel3.setText("Service Name");
        roundPanel1.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 190, -1, -1));

        save.setBackground(new java.awt.Color(153, 204, 255));
        save.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        save.setForeground(new java.awt.Color(0, 102, 204));
        save.setText("SAVE SERVICE");
        save.setBorder(null);
        save.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                saveActionPerformed(evt);
            }
        });
        roundPanel1.add(save, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 490, 340, 35));

        update.setBackground(new java.awt.Color(153, 204, 255));
        update.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        update.setForeground(new java.awt.Color(0, 102, 204));
        update.setText("UPDATE");
        update.setBorder(null);
        update.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                updateActionPerformed(evt);
            }
        });
        roundPanel1.add(update, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 550, 160, 35));

        jLabel7.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        jLabel7.setForeground(new java.awt.Color(102, 102, 102));
        jLabel7.setText("After that you can save the service to system.");
        roundPanel1.add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 700, 320, -1));

        jLabel9.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        jLabel9.setForeground(new java.awt.Color(102, 102, 102));
        jLabel9.setText("Fill all these fields then click SAVE SERVICE button, ");
        roundPanel1.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 680, 320, -1));

        jLabel14.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        jLabel14.setForeground(new java.awt.Color(102, 102, 102));
        jLabel14.setText("UPDATE or DELETE service.");
        roundPanel1.add(jLabel14, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 770, 320, -1));

        jLabel15.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        jLabel15.setForeground(new java.awt.Color(102, 102, 102));
        jLabel15.setText("Double-Click to select service using this table.");
        roundPanel1.add(jLabel15, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 730, 320, -1));

        jLabel16.setFont(new java.awt.Font("Dialog", 0, 12)); // NOI18N
        jLabel16.setForeground(new java.awt.Color(102, 102, 102));
        jLabel16.setText("Then automatically fill all these fields. After that you can ");
        roundPanel1.add(jLabel16, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 750, 320, -1));

        jLabel1.setBackground(new java.awt.Color(51, 204, 255));
        jLabel1.setOpaque(true);
        roundPanel1.add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 740, 10, 40));

        jLabel5.setBackground(new java.awt.Color(51, 204, 255));
        jLabel5.setOpaque(true);
        roundPanel1.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 680, 10, 40));

        jLabel11.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        jLabel11.setForeground(new java.awt.Color(102, 102, 102));
        jLabel11.setText("Status");
        roundPanel1.add(jLabel11, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 380, -1, -1));

        txt_amount.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        txt_amount.setForeground(new java.awt.Color(102, 102, 102));
        txt_amount.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        txt_amount.setBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 153, 204)));
        roundPanel1.add(txt_amount, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 310, 340, 35));

        jLabel4.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        jLabel4.setForeground(new java.awt.Color(102, 102, 102));
        jLabel4.setText("Service Amount");
        roundPanel1.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 280, -1, -1));

        clearField.setIcon(new javax.swing.ImageIcon(getClass().getResource("/com/burger/icon/close_1.png"))); // NOI18N
        clearField.setText("Clear Fields");
        clearField.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                clearFieldMouseClicked(evt);
            }
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                clearFieldMouseEntered(evt);
            }
        });
        roundPanel1.add(clearField, new org.netbeans.lib.awtextra.AbsoluteConstraints(240, 370, 120, 40));

        switchButton.setFont(new java.awt.Font("Dialog", 1, 18)); // NOI18N
        switchButton.setName(""); // NOI18N
        roundPanel1.add(switchButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 380, 40, 20));

        add(roundPanel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 380, 1030));

        roundPanel2.setBackground(new java.awt.Color(255, 255, 255));
        roundPanel2.setLayout(new org.netbeans.lib.awtextra.AbsoluteLayout());

        card1.setDescription("");
        card1.setIcon(javaswingdev.GoogleMaterialDesignIcon.SEARCH);
        roundPanel2.add(card1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 30, 60, 50));

        jLabel13.setFont(new java.awt.Font("Dialog", 1, 24)); // NOI18N
        jLabel13.setText("Search Service");
        roundPanel2.add(jLabel13, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 40, -1, -1));

        textFieldAnimation1.setBackground(new java.awt.Color(228, 232, 229));
        textFieldAnimation1.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyReleased(java.awt.event.KeyEvent evt) {
                textFieldAnimation1KeyReleased(evt);
            }
        });
        roundPanel2.add(textFieldAnimation1, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 110, 330, -1));

        JT_service.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "Service ID", "Service Name", "Service Amount", ""
            }
        ) {
            boolean[] canEdit = new boolean [] {
                false, false, false, false
            };

            public boolean isCellEditable(int rowIndex, int columnIndex) {
                return canEdit [columnIndex];
            }
        });
        JT_service.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                JT_serviceMouseClicked(evt);
            }
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                JT_serviceMouseEntered(evt);
            }
        });
        jScrollPane1.setViewportView(JT_service);

        roundPanel2.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 200, 1220, 800));

        add(roundPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 10, 1280, 1030));
    }// </editor-fold>//GEN-END:initComponents

    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        try {
            if (!txt_service_id.getText().equals("")) {
                int deleteSer = JOptionPane.showConfirmDialog(null, "Do you want to Delete this Service Type?", "Select", JOptionPane.YES_NO_OPTION);
                if (deleteSer == 0) {
                    ResultSet rs = DB.search("select * from service_type where service_id = '" + txt_service_id.getText() + "'");
                    if (rs.next()) {
                        DB.push("update service_type set stat = '0' where service_id ='"
                                + txt_service_id.getText() + "'");

                        DB.push("insert into system_activity (detail, date, time, employee) values ("
                                + "'Service Type Deleted',CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),'" + SystemConfigData.getEmployee() + "')");

                        JOptionPane.showMessageDialog(this, "Service Type Delete Successfuly!", "Deleted", JOptionPane.INFORMATION_MESSAGE);
//                   
                        log.info("Service Type Deleted Successful. Service Type : " + txt_service_id.getText() + " | By : " + SystemConfigData.getEmployee());
                        clearFields();
                        loadServTable();
                    } else {
                        clearFields();
                        JOptionPane.showMessageDialog(this, "Invalid Service Type ID!", "Error", JOptionPane.ERROR_MESSAGE);

                        log.error("Invalid Service Type Id");
                    }

                }
            } else {
                JOptionPane.showMessageDialog(this, "Fill the Product ID!", "Error", JOptionPane.ERROR_MESSAGE);

            }
        } catch (Exception e) {
            log.error("Service Type Delete Failed", e);

        }
    }//GEN-LAST:event_deleteActionPerformed

    private void saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveActionPerformed
        try {
            ResultSet rs = DB.search("select * from service_type where service_id ='" + txt_service_id.getText() + "' ");
            if (rs.next()) {
                JOptionPane.showMessageDialog(this, "Service ID is already Used.Please use UPDATE button to update details", "Error", JOptionPane.ERROR_MESSAGE);

            } else {
                if (!txt_service_id.getText().equals("") && !txt_service_name.getText().equals("")
                        && !txt_amount.getText().equals("")) {

                    String stat = null;
                    if (switchButton.isSelected()) {
                        stat = "1";
                    } else {
                        stat = "0";
                    }

                    DB.push("insert into service_type values('"
                            + txt_service_id.getText() + "','"
                            + txt_service_name.getText() + "','"
                            + txt_amount.getText().trim() + "','"
                            + stat + "')");

                    DB.push("insert into system_activity (detail, date, time, employee) values ("
                            + "'Service Type Saved',CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),'" + SystemConfigData.getEmployee() + "')");

                    log.info("Service Type Saved Successful. Service Type  : " + txt_service_id.getText() + " |By : " + SystemConfigData.getEmployee());
                    clearFields();
                    JOptionPane.showMessageDialog(this, "Service Type Saved Successfuly!", "Saved", JOptionPane.INFORMATION_MESSAGE);

                    loadServTable();

                } else {
                    JOptionPane.showMessageDialog(this, "Fill All text Inpute!", "Error", JOptionPane.ERROR_MESSAGE);

                }
            }
        } catch (Exception e) {
            log.error("Service Type Save Failed", e);
        }
    }//GEN-LAST:event_saveActionPerformed

    private void updateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateActionPerformed
        try {
            if (!txt_service_id.getText().equals("") && !txt_service_name.getText().equals("")
                    && !txt_amount.getText().equals("")) {
                ResultSet rs = DB.search("select * from service_type where service_id = '" + txt_service_id.getText() + "'");
                if (rs.next()) {

                    String stat = null;
                    if (switchButton.isSelected()) {
                        stat = "1";
                    } else {
                        stat = "0";
                    }

                    DB.push("update service_type set  service_name='" + txt_service_name.getText() + "'"
                            + ", amount='" + txt_amount.getText().trim() + "', "
                            + " stat='" + stat + "' "
                            + "where service_id ='"
                            + txt_service_id.getText() + "' ");
                    log.info("Service Type Updated Successful. Service Type : " + txt_service_id.getText() + " By : " + SystemConfigData.getEmployee());
                    DB.push("insert into system_activity (detail, date, time, employee) values ("
                            + "'Service Type Updated',CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),'" + SystemConfigData.getEmployee() + "')");

                    clearFields();
                    JOptionPane.showMessageDialog(this, "Service Type Updated Successfuly!", "Updated", JOptionPane.INFORMATION_MESSAGE);

                    loadServTable();

                } else {
                    JOptionPane.showMessageDialog(this, "Use SAVE button to Save Details!", "Error", JOptionPane.ERROR_MESSAGE);
                }
            } else {
                JOptionPane.showMessageDialog(this, "Fill All text Inpute!", "Error", JOptionPane.ERROR_MESSAGE);

            }
        } catch (Exception e) {
            log.error("Service Type Update Failed", e);
        }
    }//GEN-LAST:event_updateActionPerformed

    private void clearFieldMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_clearFieldMouseClicked
        clearFields();
    }//GEN-LAST:event_clearFieldMouseClicked

    private void clearFieldMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_clearFieldMouseEntered

    }//GEN-LAST:event_clearFieldMouseEntered

    private void JT_serviceMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_JT_serviceMouseClicked
        try {
            DefaultTableModel dtm = (DefaultTableModel) JT_service.getModel();
            int selectRow = JT_service.getSelectedRow();

            String ser_id = dtm.getValueAt(selectRow, 0).toString();
            txt_service_id.setText(ser_id);

            String ser_name = dtm.getValueAt(selectRow, 1).toString();
            txt_service_name.setText(ser_name);
            String ser_amount = dtm.getValueAt(selectRow, 2).toString();
            txt_amount.setText(ser_amount);

            ResultSet rs = DB.search("select * from service_type where service_id = '" + ser_id + "'");
            if (rs.next()) {

                String status = (rs.getString("stat"));
                if (status.equals("1")) {
                    switchButton.setSelected(true);
                } else {
                    switchButton.setSelected(false);
                }
            }

        } catch (Exception e) {
            log.error("Load Product Details Failed", e);
        }
    }//GEN-LAST:event_JT_serviceMouseClicked

    private void textFieldAnimation1KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_textFieldAnimation1KeyReleased
        DefaultTableModel dtm = (DefaultTableModel) JT_service.getModel();
        String searchText = textFieldAnimation1.getText();
        TableRowSorter<DefaultTableModel> searchServ = new TableRowSorter<>(dtm);
        JT_service.setRowSorter(searchServ);
        searchServ.setRowFilter(RowFilter.regexFilter(searchText));
    }//GEN-LAST:event_textFieldAnimation1KeyReleased

    private void JT_serviceMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_JT_serviceMouseEntered
        // TODO add your handling code here:
    }//GEN-LAST:event_JT_serviceMouseEntered


    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javaswingdev.swing.table.Table JT_service;
    private javaswingdev.card.Card card1;
    private javax.swing.JLabel clearField;
    private javax.swing.JButton delete;
    private javax.swing.JLabel jLabel1;
    private javax.swing.JLabel jLabel11;
    private javax.swing.JLabel jLabel13;
    private javax.swing.JLabel jLabel14;
    private javax.swing.JLabel jLabel15;
    private javax.swing.JLabel jLabel16;
    private javax.swing.JLabel jLabel2;
    private javax.swing.JLabel jLabel3;
    private javax.swing.JLabel jLabel4;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JLabel jLabel6;
    private javax.swing.JLabel jLabel7;
    private javax.swing.JLabel jLabel9;
    private javax.swing.JScrollPane jScrollPane1;
    private javaswingdev.swing.RoundPanel roundPanel1;
    private javaswingdev.swing.RoundPanel roundPanel2;
    private javax.swing.JButton save;
    private javaswingdev.swing.SwitchButton switchButton;
    private swing.TextFieldAnimation textFieldAnimation1;
    private javax.swing.JTextField txt_amount;
    private javax.swing.JTextField txt_service_id;
    private javax.swing.JTextField txt_service_name;
    private javax.swing.JButton update;
    // End of variables declaration//GEN-END:variables

    private void generateServiceID() {
        try {
            ResultSet rs = DB.search("select count(*) as ser_count from service_type");
            if (rs.next()) {
                int count = rs.getInt("ser_count");
                int c = Integer.parseInt(new SimpleDateFormat("Mddyy").format(new Date()));
                int value = c + (++count);
                String Serid = "STP-" + value;
                txt_service_id.setText(Serid);
                log.info("Service ID Loaded");
            }
            txt_service_name.grabFocus();
        } catch (Exception e) {
            log.error("Generate Service ID Failed", e);
        }
    }

    private void loadServTable() {
        try {
            DefaultTableModel dtm = (DefaultTableModel) JT_service.getModel();
            dtm.setRowCount(0);
            ResultSet rs = DB.search("select * from service_type");
            while (rs.next()) {
                Vector v = new Vector();
                v.add(rs.getString("service_id"));
                v.add(rs.getString("service_name"));
                v.add(rs.getString("amount"));
                dtm.addRow(v);
            }

        } catch (Exception e) {
            log.error("Load Service Details Failed", e);
        }
    }

    private void clearFields() {
        txt_amount.setText(null);
        txt_service_name.setText(null);
        switchButton.setSelected(false);
        generateServiceID();
    }
}
