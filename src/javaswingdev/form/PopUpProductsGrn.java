...
public class PopUpProductsGrn extends javax.swing.JFrame {

     private Logger log;
     private GoodsReceiveNote grn;
    
    public PopUpProductsGrn() {
        initComponents();
        setLocationRelativeTo(null);
        
         initLogger();
        log = Logger.getLogger("bikeLogger");
        log.info("Select Product Popup Page1 Loaded");
        table.fixTable(jScrollPane1);
        loadTable();
        
    }
    
    PopUpProductsGrn (GoodsReceiveNote grn){
        this();
        this.grn = grn;
    }
    ...
    private void tableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableMouseClicked
            ...
            String pro_id = dtm.getValueAt(selectRow, 0).toString();
            String name = dtm.getValueAt(selectRow, 1).toString();
            String brandId = dtm.getValueAt(selectRow, 2).toString();
            
            grn.txt_product_id.setText(pro_id);
            grn.txt_product_name.setText(name);
             ResultSet rs = DB.search("select * from brand where brand_id ='" + brandId + "' ");
            if (rs.next()) {
             String brand = rs.getString("brand_name");
             grn.txt_product_brand.setText(brand);
             dispose();
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PopUpProductsGrn.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new PopUpProductsGrn().setVisible(true);
            }
        });
    }
    ...
    private void loadTable() {
            ...
            DefaultTableModel dtm = (DefaultTableModel) table.getModel();
            dtm.setRowCount(0);
            ResultSet rs = DB.search("select * from product where status = '1'");
            while (rs.next()) {
                Vector v = new Vector();
